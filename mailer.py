...
def send_mail(to_email, subject, text_body, html_body=None):
    ...
    try:
        ...
        if error_code in bounce_error_codes or response.status_code == 422:
            print(f"[BOUNCE] Adding {to_email} to bounce list due to error {error_code}")
...
def test_email_system():
    ...
    print(f"POSTMARK_SERVER_TOKEN: {'Set' if os.getenv('POSTMARK_SERVER_TOKEN') else 'Missing'}")
